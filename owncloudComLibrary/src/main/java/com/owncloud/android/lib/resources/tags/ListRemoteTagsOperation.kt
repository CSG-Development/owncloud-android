...
class ListRemoteTagsOperation : RemoteOperation<List<RemoteTag>>() {
    ...
    override fun run(client: OwnCloudClient): RemoteOperationResult<List<RemoteTag>> {
        ...
        return try {
            ...
            if (isSuccess(status)) {
                val tags = propfindMethod.members.map { response ->
                    parseTag(response)
                }
                RemoteOperationResult<List<RemoteTag>>(ResultCode.OK).apply {
                    data = tags
                    Timber.i("Listed ${tags.size} tags - HTTP status code: $status")
                }
            } else {
                RemoteOperationResult<List<RemoteTag>>(propfindMethod).also {
                    Timber.w("List tags failed: ${it.logMessage}")
                }
            }
        } catch (e: Exception) {
            RemoteOperationResult<List<RemoteTag>>(e).also {
                Timber.e(it.exception, "Exception while listing tags")
            }
        }
    }
