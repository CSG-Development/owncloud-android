...
import com.owncloud.android.presentation.files.filelist.FileListSelectionIds

/**
 * Compose-facing id-based selection holder (Previews / future hosts).
 * Production ViewModels use [FileListSelectionIds] on a [kotlinx.coroutines.flow.MutableStateFlow].
 */
@Stable
class FileListSelectionState(
    ...
    fun select(fileId: Long) {
        selectedIds = FileListSelectionIds.select(selectedIds, fileId)
    }
    ...
    fun toggle(fileId: Long) {
        selectedIds = FileListSelectionIds.toggle(selectedIds, fileId)
    }
    ...
    fun selectAll(fileIds: Collection<Long>) {
        selectedIds = FileListSelectionIds.selectAll(fileIds)
    }
    ...
    fun inverse(fileIds: Collection<Long>) {
        selectedIds = FileListSelectionIds.inverse(selectedIds, fileIds)
    }

    /** Drops selected ids that are not present in [fileIds] (e.g. after folder/filter change). */
    fun retain(fileIds: Collection<Long>) {
        val retained = FileListSelectionIds.retain(selectedIds, fileIds)
